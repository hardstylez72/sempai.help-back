...
router.post('/', function(req, res) {
    if ((req.body.imgSrc !== undefined) && (req.body.imgType !== undefined)) {
        ...
        res.json(JSON.stringify({answ: "err"}));
});
        ...
        function isImgAlreadyExistCallBack(err, data) {
            if (err) 
                ...
                if (i === files.length)
                    return cb(null, false);
                i++;
                let hashBuffer = crypto.createHash('md5').update(data).digest('hex');
                if (hashBuffer === hash)
                    return cb(null, true);
                else
...
function getFilesWithProps(path, type, cb) {
    if ((path == undefined) || (type == undefined) || (cb == undefined))
        return cb('Input params are not valid', null);
    fs.readdir(path, (err, files) => {
        if (err) 
            return cb(err, null);
        else {
            var filesFound = files.filter((file) => {
            if (file.search(RegExp('\.' + type)) !== -1)
                return file;
            else if (type === 'jpeg') 
                if (file.search(RegExp('.jpg')) !== -1)
                    return file;
            else if  (type === 'jpg')
                if (file.search(RegExp('.jpeg')) !== -1)
                        return file;
            });
